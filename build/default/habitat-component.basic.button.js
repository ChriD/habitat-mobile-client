...
class HabitatComponent_Basic_Button extends LitElement {
  ...
  dispatchClick(_longPressClickIntervalCount) {
    this.dispatchEvent(new CustomEvent('clicked', {
      detail: {
        count: _longPressClickIntervalCount
      }
    }));
  }
  ...
  _onDown(_e) {
    var self = this;
    console.log(_e.type); // on a new down event we have to clear the exiating interval
    // in fact this should never be the case because the 'onUp' will clear the interval too

    if (self.longPressIntervalId) clearInterval(self.longPressIntervalId); // if the user presses the button and we do have some long click interval defined we
  ...
  _onUp(_e) {
    console.log(_e.type); // clear the intervall and dispacth the last click

    if (this.longPressIntervalId) clearInterval(this.longPressIntervalId);
    this.dispatchClick(this.longPressClickIntervalCount);
    this.longPressClickIntervalCount = 0; //_e.stopPropagation()
